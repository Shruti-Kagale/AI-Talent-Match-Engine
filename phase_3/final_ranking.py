...
OUT_FINAL = ARTIFACTS / "final_ranking.xlsx"
...
# ----------------------------
# Create final ranking with threshold
# ----------------------------
def create_final_ranking(top_n=TOP_N):
    ...
    filtered = merged[merged["final_score"] >= MATCH_THRESHOLD]
    filtered = filtered.sort_values(["job_id","final_score"], ascending=[True,False])
    filtered = filtered.groupby("job_id").head(top_n)
    filtered["rank"] = filtered.groupby("job_id")["final_score"].rank(ascending=False, method="first").astype(int)

    filtered.to_excel(OUT_FINAL, index=False)
    print(f"[OK] Final ranking → {OUT_FINAL}")
